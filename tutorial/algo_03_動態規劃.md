...
想像你在爬樓梯，每次可以爬 1 階或 2 階，要算爬到第 n 階有幾種方法。你寫了一個遞迴：
...
看起來很漂亮對吧？但如果你把 `climb(5)` 的遞迴呼叫畫成樹：
...
你會發現 `climb(3)` 被算了 2 次、`climb(2)` 被算了 3 次！當 n 很大時，重複計算的次數爆炸性成長，程式慢到不行（時間是 $O(2^n)$）。

**動態規劃（DP）的核心直覺**：**把大問題拆成小問題，然後把小問題的答案「記住」，避免重複計算。**
...
for i in range(2, n+1):
    dp[i] = dp[i-1] + dp[i-2]    # 直接查表，不用重算！
```

這樣每個子問題只算一次，時間變成 $O(n)$，比 $O(2^n)$ 快了幾百萬倍！

### 用 Fibonacci 理解「重複子問題」

Fibonacci 數列：$F(0)=0, F(1)=1, F(n)=F(n-1)+F(n-2)$

| 方法 | 時間 | 為什麼 |
|------|------|--------|
| 純遞迴 | $O(2^n)$ | 同一個子問題被重複算指數多次 |
| DP（記住答案） | $O(n)$ | 每個子問題只算一次，之後直接查表 |

### 怎麼看出一題是 DP 題？判斷標準

一個問題適合用 DP，通常有以下兩個特徵：

**特徵 1：最佳子結構（Optimal Substructure）**
> 大問題的最佳解可以由小問題的最佳解組成。
> - 例：「爬到第 5 階的方法數 = 爬到第 4 階的方法數 + 爬到第 3 階的方法數」

**特徵 2：重疊子問題（Overlapping Subproblems）**
> 把問題拆開後，不同的拆法會遇到「同一個小問題」。
> - 例：算 $F(5)$ 時要算 $F(3)$，算 $F(4)$ 時也要算 $F(3)$ → $F(3)$ 被重複需要

**快速判斷口訣**：
- 題目問「最多/最少/方法數」→ 很可能是 DP
- 題目有「選或不選」的決策 → 很可能是 DP
- 你畫出遞迴樹發現有重複的節點 → 一定要用 DP
- 如果子問題之間是獨立的（沒有重疊）→ 用分治法而非 DP

### Top-down vs Bottom-up：兩種實作方式

| | Top-down（記憶化搜尋） | Bottom-up（填表法） |
|---|---|---|
| **做法** | 還是寫遞迴，但把算過的答案存在表裡 | 直接用迴圈從小問題開始填表 |
| **方向** | 從大問題出發，需要時才算小問題 | 從最小的子問題開始，一路算到大問題 |
| **優點** | 程式碼跟遞迴式一模一樣，好寫 | 沒有遞迴呼叫的額外開銷，通常更快 |
| **缺點** | 遞迴太深可能 stack overflow | 要自己想清楚填表順序 |
| **比喻** | 「需要什麼才去查，查過的記下來」 | 「按順序把所有答案算好，最後直接查」 |

### 本章關鍵術語表

| 術語 | 英文 | 白話解釋 | 例子 |
|------|------|----------|------|
| 動態規劃 | Dynamic Programming (DP) | 把大問題拆成小問題，記住答案避免重複計算 | 背包問題、LCS |
| 最佳子結構 | Optimal Substructure | 大問題的最佳解包含小問題的最佳解 | 最短路徑問題 |
| 重疊子問題 | Overlapping Subproblems | 不同的拆法會遇到同一個小問題 | Fibonacci 的遞迴樹 |
| 狀態 | State | DP 表格中每一格代表一個子問題 | dp[i][w] = 前 i 個物品、容量 w |
| 轉移方程 | Recurrence / Transition | 大問題的答案如何由小問題的答案算出 | dp[i] = dp[i-1] + dp[i-2] |
| 基底情況 | Base Case | 最小的子問題，直接知道答案 | dp[0] = 0 |
| 記憶化 | Memoization | 把算過的答案存在表裡，下次直接查 | Top-down DP 的做法 |
| 製表法 | Tabulation | 用迴圈從小到大填表 | Bottom-up DP 的做法 |
| 背包問題 | Knapsack Problem | 有限容量的背包要裝最多價值的物品 | 見 5.1 節 |
| 偽多項式 | Pseudo-polynomial | 看起來是多項式但其實依賴於數值大小 | $O(nW)$ 中 W 是數值 |

### 前置知識

- **程式基礎**：理解遞迴、迴圈、二維陣列
- **分治法基礎**（algo_02）：理解「拆成子問題再合併」的概念
- **基礎數學**：$\max$ 和 $\min$ 的意義
...
#### Step-by-step 填表範例（每一格都詳細說明）

> **給初學者**：DP 表格的每一格 dp[i][w] 代表一個問題的答案：「如果只有前 i 個物品可以選，而背包容量是 w，最多能裝多少價值？」
>
> 填每一格時你只需要問自己一個問題：**第 i 個物品要不要放進去？**
> - **不放**：答案跟「只有前 i-1 個物品、容量 w」一樣 → dp[i-1][w]
> - **放**：先騰出 $w_i$ 的空間，再加上物品 i 的價值 → dp[i-1][w - $w_i$] + $v_i$
> - 取兩者的較大值
...
> ```
>
> - dp[1][1]：X[1]='A' vs Y[1]='B' → 不同 → max(dp[0][1], dp[1][0]) = max(0, 0) = 0
> - dp[1][2]：X[1]='A' vs Y[2]='C' → 不同 → max(dp[0][2], dp[1][1]) = max(0, 0) = 0
> - dp[2][1]：X[2]='C' vs Y[1]='B' → 不同 → max(dp[1][1], dp[2][0]) = max(0, 0) = 0
> - dp[2][2]：X[2]='C' vs Y[2]='C' → **相同！** → dp[1][1] + 1 = 0 + 1 = 1
>
> LCS = "C"，長度 1。看到了嗎？當字元相同時，往左上角 +1；不同時，取上面或左邊的較大值。
...
1. **用自己的話解釋**：什麼是「重疊子問題」？為什麼有重疊子問題的時候用純遞迴會很慢？

2. **DP vs 分治法**：Merge Sort 的子問題有重疊嗎？為什麼 Merge Sort 用分治法而不是 DP？

3. **判斷**：以下問題是 DP 題還是 Greedy 題？
   - (a) 0/1 背包問題（物品只能整個拿或不拿）
   - (b) 分數背包問題（物品可以拿一部分）
   - (c) 活動選擇問題（選最多不重疊的活動）
   - (d) 最長公共子序列

4. **Top-down vs Bottom-up**：在什麼情況下你會選 Top-down？什麼情況選 Bottom-up？

### 計算題

5. **Fibonacci DP 填表**：用 bottom-up 方式算出 $F(0)$ 到 $F(7)$ 的值。

6. **0/1 Knapsack 填表**：物品如下，背包容量 W = 6。請填出完整的 dp 表格。
   | 物品 | 重量 | 價值 |
   |------|------|------|
   | 1    | 2    | 3    |
   | 2    | 3    | 5    |
...
7. **LCS 填表**：X = "CAT", Y = "ACT"。填出完整的 dp 表格，並回溯找出 LCS。

8. **Coin Change**：硬幣面額 {1, 3, 4}，金額 n = 7。用 DP 填表求最少硬幣數，並寫出使用了哪些硬幣。

9. **子問題定義**：如果 LIS（最長遞增子序列）定義 dp[i] = 「前 i 個元素的 LIS 長度」（而非「以 A[i] 結尾的 LIS 長度」），請解釋為什麼這個定義推不出遞迴式。

### 參考答案提示

- 第 3 題：(a) DP (b) Greedy (c) Greedy (d) DP
- 第 5 題：$F(0)=0, F(1)=1, F(2)=1, F(3)=2, F(4)=3, F(5)=5, F(6)=8, F(7)=13$
- 第 7 題：LCS = "CT" 或 "AT"，長度 2
- 第 8 題：dp = [0, 1, 2, 1, 1, 2, 2, 2]，dp[7] = 2（使用 3+4 或 4+3）
- 第 9 題：因為知道「前 i-1 個的 LIS 長度」時，你不知道 LIS 的最後一個元素是什麼，所以無法判斷 A[i] 能不能接上去
