...
條件機率就是「在已經知道某件事的前提下，另一件事發生的可能性有多大」。

生活中的例子：
- 今天出門看到天空烏雲密布（已知訊息），那等一下下雨的機率有多大？
- 你的朋友咳嗽了（已知訊息），他感冒的機率有多大？
- 你拿到了一張陽性的檢測報告（已知訊息），你真的有那個病的機率有多大？

重點是：**當你得到新的資訊之後，對事情的判斷會跟著更新**。

舉個簡單例子：從一副 52 張撲克牌裡抽一張，抽到紅心的機率是 13/52 = 1/4。但如果有人偷看告訴你「這張是紅色的」（紅心或方塊），那在這個新資訊下，抽到紅心的機率就變成 13/26 = 1/2。因為你的「世界」從 52 張牌縮小成了 26 張紅色牌。

### 你需要知道的數學基礎

本章需要的數學很簡單：
1. **分數的乘除**：條件機率的核心公式就是一個分數
2. **加法與乘法**：全概率公式就是「分情況相加」
3. **第一章的機率概念**：事件、樣本空間、$P(A \cup B)$ 等
...
| 術語 | 英文 | 白話解釋 | 例子 |
|------|------|----------|------|
| 條件機率 | Conditional Probability | 在已知某事下，另一事的機率 | 已知下雨，帶傘的機率 |
| 乘法規則 | Multiplication Rule | 算「兩件事同時發生」的公式 | 連續抽兩張都是紅心 |
| 全概率公式 | Law of Total Probability | 把事情分情況加總 | 不管晴天雨天，遲到的總機率 |
| 貝氏定理 | Bayes' Theorem | 看到結果，反推原因 | 檢測陽性，反推真的有病嗎 |
| 先驗機率 | Prior Probability | 觀察之前的初始判斷 | 某疾病在人群中的盛行率 |
| 似然度 | Likelihood | 若原因為真，看到此結果的機會 | 若真的有病，檢測呈陽性的機率 |
| 後驗機率 | Posterior Probability | 觀察之後更新的判斷 | 檢測陽性後，真的有病的機率 |
| 獨立 | Independent | 兩事件互不影響 | 丟兩顆骰子，各自的點數 |
| 互斥 | Mutually Exclusive | 兩事件不能同時發生 | 骰子出 1 和骰子出 2 |
| Base Rate | 基率 | 某事在總人口中的基本比率 | 疾病的盛行率 |
...
> **📝 數值範例（下雨帶傘）**：某城市的天氣統計顯示：下雨的日子佔全年的 30%，出門帶傘的日子佔全年的 40%，而「下雨且帶傘」的日子佔全年的 25%。問：如果今天下雨了，你帶了傘的機率是多少？
>
> 步驟 1：定義事件。$A$ = 帶傘，$B$ = 下雨
> 步驟 2：已知 $P(B) = 0.30$，$P(A) = 0.40$，$P(A \cap B) = 0.25$
> 步驟 3：套公式 $P(A|B) = \frac{P(A \cap B)}{P(B)} = \frac{0.25}{0.30} = \frac{5}{6} \approx 0.833$
>
> 答案：下雨天帶傘的機率是 83.3%
>
> 注意：$P(B|A) = \frac{P(A \cap B)}{P(A)} = \frac{0.25}{0.40} = 0.625$（帶傘時下雨的機率是 62.5%），和 $P(A|B)$ 不同！
...
> **📝 數值範例**：袋中有 3 紅球、2 白球。不放回地連續抽 2 球，兩球都是紅色的機率？
>
> 步驟 1：$P(\text{第一紅}) = 3/5$
> 步驟 2：$P(\text{第二紅}|\text{第一紅}) = 2/4 = 1/2$（抽走一個紅球後，剩下 2 紅 2 白）
> 步驟 3：$P(\text{兩紅}) = P(\text{第一紅}) \times P(\text{第二紅}|\text{第一紅}) = 3/5 \times 1/2 = 3/10 = 0.3$
>
> 答案：兩球都是紅色的機率是 30%
...
> **📝 數值範例（全概率公式步步走）**：
>
> 步驟 1：辨認「分割」——天氣分成兩種情況：下雨（40%）和不下雨（60%），互斥且窮舉。
> 步驟 2：寫出每種情況下遲到的機率——下雨 → 50% 遲到，不下雨 → 10% 遲到。
> 步驟 3：分別相乘——$0.5 \times 0.4 = 0.20$（下雨且遲到），$0.1 \times 0.6 = 0.06$（不下雨且遲到）。
> 步驟 4：相加——$0.20 + 0.06 = 0.26$。
>
> 答案：明天遲到的總機率是 26%。
>
> 白話：全概率公式就是「把所有可能的情況都想一遍，每種情況的機率乘以在那種情況下發生的機率，最後加起來」。
...
> **📝 數值範例（醫學檢測完整拆解）**：假設某種罕見疾病在人群中的盛行率是 1%（每 100 人有 1 人有病）。有一種檢測方法：
> - 如果真的有病，檢測呈陽性的機率是 95%（靈敏度）
> - 如果沒有病，檢測呈陰性的機率是 90%（特異度），換言之誤判為陽性的機率是 10%
>
> 問：一個人檢測結果呈陽性，他真的有病的機率是多少？
>
> **步驟 1：辨認各個角色**
> - **先驗機率**（Prior）：$P(\text{有病}) = 0.01$（還沒做檢測之前，你對他有病的相信程度）
> - **似然度**（Likelihood）：$P(\text{陽性} | \text{有病}) = 0.95$（如果真的有病，看到陽性結果的機會）
> - 我們要求的是**後驗機率**（Posterior）：$P(\text{有病} | \text{陽性})$（看到陽性結果之後，更新的判斷）
>
> **步驟 2：先用全概率公式算分母（證據）**
> $P(\text{陽性}) = P(\text{陽性}|\text{有病}) \times P(\text{有病}) + P(\text{陽性}|\text{沒病}) \times P(\text{沒病})$
> $= 0.95 \times 0.01 + 0.10 \times 0.99 = 0.0095 + 0.099 = 0.1085$
>
> **步驟 3：套貝氏定理**
> $P(\text{有病}|\text{陽性}) = \frac{0.95 \times 0.01}{0.1085} = \frac{0.0095}{0.1085} \approx 0.0876$
>
> **答案：即使檢測呈陽性，真的有病的機率也只有約 8.8%！**
>
> 為什麼這麼低？因為沒病的人太多了（99%），即使誤判率只有 10%，誤判的「人數」還是比真陽性的人數多得多。用 10000 人想像：100 人有病、其中 95 人陽性；9900 人沒病、其中 990 人誤判陽性。陽性總共 1085 人，其中只有 95 人真的有病。
...
> **📝 數值範例（丟兩個骰子驗證獨立性）**：丟兩顆公平骰子。令 $A$ = 第一顆是 3，$B$ = 第二顆是 5。$A$ 和 $B$ 是否獨立？
>
> 步驟 1：$P(A) = 1/6$（第一顆出 3）
> 步驟 2：$P(B) = 1/6$（第二顆出 5）
> 步驟 3：$P(A \cap B) = P(\text{第一顆 3 且第二顆 5}) = 1/36$（36 種結果中只有 (3,5) 這一種）
> 步驟 4：驗證 $P(A) \times P(B) = 1/6 \times 1/6 = 1/36 = P(A \cap B)$ ✓
>
> 答案：$A$ 和 $B$ 獨立。這很直觀：第一顆骰子出什麼，完全不影響第二顆骰子出什麼。
>
> **反例**：如果是同一顆骰子，$A$ = 出偶數，$C$ = 出大於 4。
> $P(A) = 3/6 = 1/2$，$P(C) = 2/6 = 1/3$，$P(A \cap C) = P(\{6\}) = 1/6$。
> $P(A) \times P(C) = 1/2 \times 1/3 = 1/6 = P(A \cap C)$ ✓，所以也是獨立！
> 這告訴我們：獨立不一定需要「物理上分開」，而是看數值關係。
...
> **📝 Monty Hall 步步拆解：為什麼換門比較好？**
>
> 很多人直覺覺得「反正剩兩扇門，機率應該各 50%」，但這是錯的！關鍵在於**主持人知道答案**。
>
> **用三種情境來想**（假設你選了 A 門）：
>
> | 情境 | 車在哪 | 機率 | 主持人開哪扇 | 不換（堅持 A） | 換門 |
> |------|--------|------|-------------|---------------|------|
> | 1 | A 門 | 1/3 | B 或 C（隨機） | 贏 ✓ | 輸 ✗ |
> | 2 | B 門 | 1/3 | C 門（只能開 C） | 輸 ✗ | 贏 ✓ |
> | 3 | C 門 | 1/3 | B 門（只能開 B） | 輸 ✗ | 贏 ✓ |
>
> **統計**：
> - 不換：3 種情境中只有 1 種贏 → 勝率 1/3
> - 換門：3 種情境中有 2 種贏 → 勝率 2/3
>
> **關鍵**：在情境 2 和 3 中，主持人被迫只能開特定的一扇門（因為不能開有車的門），所以他的行為「洩漏了資訊」。這個額外的資訊讓換門變得有利。
>
> 如果主持人是隨機開門（不知道車在哪），那就真的是 50/50 了。正是因為他「知道答案」，才造成了 2/3 vs 1/3 的不對稱。
...
1. **（觀念題）** 「$P(\text{感冒}|\text{咳嗽})$」和「$P(\text{咳嗽}|\text{感冒})$」哪個通常比較大？為什麼？請用一兩句話解釋。

2. **（計算題）** 一個袋子裡有 4 顆紅球和 6 顆白球。不放回地依序抽出 2 顆。(a) 求兩顆都是紅球的機率。(b) 已知第二顆是紅球，求第一顆也是紅球的機率。

3. **（進階應用題）** 工廠有兩條生產線：A 線生產 60% 的產品，不良率 2%；B 線生產 40% 的產品，不良率 5%。隨機取一個產品發現是不良品，它來自 A 線的機率是多少？（提示：用貝氏定理）

<details>
<summary>參考答案</summary>

1. $P(\text{咳嗽}|\text{感冒})$ 通常比較大。因為感冒的人「大部分」會咳嗽，但咳嗽的人不一定是感冒（可能是過敏、空氣不好等原因）。這是典型的 $P(A|B) \neq P(B|A)$ 的例子。

2. (a) $P(\text{兩紅}) = P(\text{第一紅}) \times P(\text{第二紅}|\text{第一紅}) = \frac{4}{10} \times \frac{3}{9} = \frac{12}{90} = \frac{2}{15} \approx 0.133$

   (b) 用貝氏定理：先算 $P(\text{第二紅})$。
   $P(\text{第二紅}) = P(\text{第二紅}|\text{第一紅})P(\text{第一紅}) + P(\text{第二紅}|\text{第一白})P(\text{第一白})$
   $= \frac{3}{9} \times \frac{4}{10} + \frac{4}{9} \times \frac{6}{10} = \frac{12}{90} + \frac{24}{90} = \frac{36}{90} = \frac{4}{10}$

   $P(\text{第一紅}|\text{第二紅}) = \frac{P(\text{第二紅}|\text{第一紅}) \times P(\text{第一紅})}{P(\text{第二紅})} = \frac{3/9 \times 4/10}{4/10} = \frac{3}{9} = \frac{1}{3}$

3. $P(A|\text{不良}) = \frac{P(\text{不良}|A) \times P(A)}{P(\text{不良}|A) \times P(A) + P(\text{不良}|B) \times P(B)}$
   $= \frac{0.02 \times 0.60}{0.02 \times 0.60 + 0.05 \times 0.40} = \frac{0.012}{0.012 + 0.020} = \frac{0.012}{0.032} = 0.375$

   來自 A 線的機率是 37.5%。雖然 A 線產量佔 60%，但因為 A 線不良率低，所以不良品中來自 A 線的反而少於一半。
