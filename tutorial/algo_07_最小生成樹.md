...
**場景：** 你是一家電信公司的工程師，要在 6 個城市之間鋪設光纖網路。任何兩個城市之間都可以鋪，但每條光纖的成本不同。你的目標是：**讓所有城市都連通（任意兩城市之間可以通訊），同時鋪設的總成本最低。**

你不需要鋪「所有」可能的光纖——那太浪費了。你只要鋪**剛好足夠**的光纖讓所有城市連通就好。6 個城市只需要 5 條光纖（一般是 V-1 條邊），而且要挑成本最低的組合。

這就是**最小生成樹 (MST)** 問題：在一個帶權無向圖中，找一棵包含所有頂點、邊權總和最小的樹。

### 本章關鍵術語表

| 術語 | 英文 | 白話解釋 | 例子 |
|------|------|----------|------|
| 生成樹 | Spanning Tree | 包含圖中所有頂點、恰好 V-1 條邊、連通且無環的子圖 | 6 個城市用 5 條光纖全部連起來（不形成環路） |
| 最小生成樹 | MST (Minimum Spanning Tree) | 邊權總和最小的生成樹 | 鋪設成本最低的光纖方案 |
| 切割 | Cut (S, V\\S) | 把所有頂點分成兩組 S 和 V\\S | 把 6 個城市分成「北部城市」和「南部城市」兩組 |
| 跨越邊 | Crossing edge | 一端在 S、另一端在 V\\S 的邊 | 一條連接北部城市和南部城市的光纖 |
| Cut Property | Cut Property（切割性質） | 跨越某個切割的最輕邊一定在 MST 中 | 連接南北兩組城市最便宜的那條光纖，一定要鋪 |
| Cycle Property | Cycle Property（環性質） | 環中最重的邊一定不在 MST 中 | 如果三條路形成環路，最貴那條可以不鋪 |
| Union-Find | Union-Find (Disjoint Set Union) | 用來追蹤「哪些元素在同一組」的資料結構 | 追蹤哪些城市已經被光纖連在一起了 |

### 前置知識

- **圖的基本概念**（algo_01~02）：無向圖、連通性、樹的定義
- **排序**（algo_01）：Kruskal 需要對邊排序
- **Priority Queue**（algo_01~02）：Prim 需要最小優先佇列
- **Greedy 演算法**（algo_04）：Kruskal 和 Prim 都是 Greedy 策略

### Cut Property 和 Cycle Property 的直覺解釋

> **Cut Property 白話版：** 想像你把所有城市分成兩組（比如河的兩岸）。要讓兩岸能通訊，至少要在河上架一條光纖橋。既然至少要架一條，那當然挑**最便宜的**那條！這就是 Cut Property 的精神——跨越分隔線的最輕邊，一定在 MST 裡。
>
> ```
> 【Cut Property 示意圖】
>
...
>         │                │
>         ├── 邊1 (成本 5)──┤
>         ├── 邊2 (成本 2)──┤  ← 最輕跨越邊，一定在 MST 中！
>         └── 邊3 (成本 8)──┘
> ```
>
> **Cycle Property 白話版：** 如果你鋪的光纖形成了一個環路（比如 A→B→C→A），那環路裡有一條是多餘的——拿掉它，三個城市仍然連通。既然要拿掉一條，當然拿掉**最貴的**那條！這就是 Cycle Property——環中最重的邊，一定不在 MST 裡。

### Union-Find 為什麼需要？（從頭解釋）

在 Kruskal 演算法中，我們按邊的權重從小到大逐條加入邊。但**加入一條邊之前，要先檢查：這條邊的兩端是不是已經連通了？** 如果已連通，加入這條邊就會形成環，必須跳過。

**問題：** 如何快速判斷「兩個點是否已經連通」？

**笨方法：** 每次都跑一遍 BFS/DFS。太慢了！

**聰明方法：** 用 **Union-Find** 資料結構！它把已連通的頂點分到同一組，支援兩個操作：
- **FIND(x)**：查 x 屬於哪一組（回傳該組的代表）
- **UNION(x, y)**：把 x 和 y 所在的組合併

每次加入邊 (u, v) 前，做 `FIND(u)` 和 `FIND(v)`：
- 如果回傳值相同 → u 和 v 已連通 → 跳過這條邊
- 如果回傳值不同 → u 和 v 還沒連通 → 加入這條邊，然後 `UNION(u, v)`

**兩個關鍵優化（讓每次操作幾乎 O(1)）：**

1. **Union by Rank：** 合併兩棵樹時，把矮的接到高的下面（防止樹退化成一條鏈）
2. **Path Compression：** 查詢時，順手把路上的節點全部直接接到根（下次查更快）

```
【Path Compression 圖解】

    查 FIND(5) 前：         查 FIND(5) 後：
        1                       1
        |                    / | \ \
        2                   2  3  4  5
        |                  （所有節點直接指向根）
...
1. **用自己的話解釋 Cut Property。** 不要背定義，用「光纖鋪設」的比喻說出核心概念。

2. **用自己的話解釋 Cycle Property。** 為什麼環中最重的邊一定不在 MST 中？

3. **Kruskal 和 Prim 的核心差異是什麼？** 分別是什麼「視角」在做 Greedy？

4. **Union-Find 的兩個優化分別解決什麼問題？** 如果不做 Path Compression，最壞情況的 FIND 時間是多少？

5. **如果所有邊權都不同，MST 唯一嗎？** 如果有邊權相同呢？

### 計算題

6. 對以下無向圖，手動執行 Kruskal 演算法。寫出每步考慮的邊、FIND 結果、是否加入、Union-Find 集合狀態。
   ```
   頂點：{A, B, C, D, E}
   邊：(A,B):3, (A,C):1, (B,C):5, (B,D):4, (C,D):2, (C,E):6, (D,E):3
   ```

7. 對同一張圖，手動執行 Prim 演算法（起點 A）。寫出每步 Extract-Min 的頂點、key[] 更新情況、加入的 MST 邊。

8. 一張圖的 MST 總權重為 15。現在加入一條新邊 (u, v) 權重為 3。說明如何用 O(V) 時間判斷 MST 是否需要更新，並描述更新方式。

### 判斷題（True/False，並說明理由）

9. 「MST 的邊權可以為負數。」

10. 「MST 不等於最短路樹。」（提示：找一個具體的反例）

11. 「可以用 Divide and Conquer 來求 MST：把頂點分成兩半，各自求 MST，再用最輕跨越邊連接。」

12. 「在 Prim 中，更新 key[v] 時應該用 `key[v] = key[u] + w(u,v)`。」

### 參考答案提示

- 第 6 題：排序後順序為 (A,C):1, (C,D):2, (A,B):3, (D,E):3, (B,D):4, (B,C):5, (C,E):6。MST = {(A,C):1, (C,D):2, (A,B):3, (D,E):3}，總權重 = 9。
- 第 9 題：True。MST 演算法對負權邊完全沒問題（和最短路不同）。
- 第 11 題：False。參見 6.4 節的反例。
- 第 12 題：False。Prim 的更新應該是 `key[v] = w(u,v)`（只看這條邊本身的權重），不是像 Dijkstra 那樣累加。
