...
之前我們學的隨機變數都是「一次性」的——丟一次骰子、抽一次樣本。但現實中，很多事情是隨著**時間演變**的：

- 便利商店每小時來幾個客人？（Poisson Process）
- 今天天氣是晴天，明天呢？後天呢？（Markov Chain）
- 賭徒的資金怎麼隨著時間變化？什麼時候會破產？（Random Walk / Gambler's Ruin）

**隨機過程**就是研究這些「隨時間變化的隨機現象」的工具。

### 本章關鍵術語表

| 術語 | 英文 | 白話解釋 | 例子 |
|------|------|----------|------|
| 隨機過程 | Stochastic Process | 一族隨著時間（或空間）變化的隨機變數 | 股票價格隨時間的變化 |
| Poisson 過程 | Poisson Process | 事件以固定平均速率隨機發生的模型 | 每小時平均來 5 個客人 |
| 到達率 | Rate (λ) | 平均每單位時間發生的事件數 | λ = 3 人/小時 |
| 間隔時間 | Inter-arrival Time | 兩次事件之間的等待時間 | 兩個客人之間的時間間隔 |
| Markov 鏈 | Markov Chain | 「未來只和現在有關，和過去無關」的隨機過程 | 天氣預測：明天只取決於今天 |
| 轉移矩陣 | Transition Matrix | 記錄所有「從狀態 i 到狀態 j」的機率 | 晴天→雨天的機率 = 0.3 |
| 穩態分布 | Stationary Distribution | 系統長期平衡後各狀態的機率 | 長期而言 60% 晴天、40% 雨天 |
| 吸收態 | Absorbing State | 一旦進入就無法離開的狀態 | 賭徒的「破產」= 錢變 0 |
| 常返 | Recurrent | 從某狀態出發，一定會再回來 | 一維隨機遊走：一定會回到原點 |
| 暫態 | Transient | 從某狀態出發，有可能永遠不回來 | 三維隨機遊走：可能迷路 |
| 熵 | Entropy | 衡量隨機變數的「不確定性」 | 公平硬幣的熵最大 = 1 bit |

### 前置知識

你需要先讀完：
- **prob_04**：熟悉 Poisson 分布和 Exponential 分布（Poisson Process 的核心）
- **prob_05**：知道聯合分布和條件分布
- **prob_06**：了解期望值的計算

關鍵公式複習：
- Poisson 分布：$P(X=k) = \frac{\lambda^k e^{-\lambda}}{k!}$
- Exponential 分布：$f(x) = \lambda e^{-\lambda x}$，$E[X] = 1/\lambda$
...
> **便利商店客人的例子——用白話理解三個條件**
>
> 設便利商店的客人以 Poisson Process 到達，速率 $\lambda = 5$ 人/小時。
>
> **條件 1：N(0) = 0**——開始計數之前，還沒來任何人。這只是起點設定。
>
> **條件 2：獨立增量**——上午 9 點到 10 點來了幾個人，和下午 2 點到 3 點來了幾個人，**彼此獨立**。白話：「不同時段的客流量互不影響。」
>
> **條件 3：平穩增量且為 Poisson**——任意一個 $t$ 小時的時段，來客數 $\sim \text{Poisson}(\lambda t) = \text{Poisson}(5t)$。白話：「不管你從什麼時候開始看，一小時內平均來 5 個人，而且實際人數服從 Poisson 分布。」
>
> **具體計算**：
>
> - 1 小時內的來客數 $N(1) \sim \text{Poisson}(5)$
>   - $P(\text{1小時內沒客人}) = e^{-5} \approx 0.0067$
>   - $P(\text{1小時內恰好5人}) = \frac{5^5 e^{-5}}{5!} = \frac{3125 \times 0.0067}{120} \approx 0.1755$
>
> - 30 分鐘（= 0.5 小時）內的來客數 $N(0.5) \sim \text{Poisson}(2.5)$
>   - $P(\text{半小時無人}) = e^{-2.5} \approx 0.0821$
>
> - 兩個客人之間的等待時間 $T \sim \text{Exp}(5)$
>   - $E[T] = 1/5 = 0.2$ 小時 = **12 分鐘**
>   - 等超過 20 分鐘的機率：$P(T > 1/3) = e^{-5/3} \approx 0.189$
...
> **天氣的例子——Markov Chain 入門**
>
> 假設天氣只有兩種狀態：**晴天 (S)** 和**雨天 (R)**。
>
> 規則：
> - 如果今天是晴天，明天有 80% 機率還是晴天，20% 機率下雨
> - 如果今天是雨天，明天有 40% 機率轉晴，60% 機率繼續下雨
>
> 這就是一個 Markov Chain！明天的天氣**只取決於今天**（不管昨天、前天怎樣）。
>
> **2x2 轉移矩陣**：
>
> $$\mathbf{P} = \begin{pmatrix} 0.8 & 0.2 \\ 0.4 & 0.6 \end{pmatrix} \quad \text{(行：今天的狀態；列：明天的狀態)}$$
>
> 讀法：第 1 行 = 「今天晴天」→ [明天晴 0.8, 明天雨 0.2]
>
> **問題**：如果今天是晴天，後天（兩天後）的天氣機率？
>
> 用 $\mathbf{P}^2$：
> $$\mathbf{P}^2 = \begin{pmatrix} 0.8 & 0.2 \\ 0.4 & 0.6 \end{pmatrix} \times \begin{pmatrix} 0.8 & 0.2 \\ 0.4 & 0.6 \end{pmatrix} = \begin{pmatrix} 0.72 & 0.28 \\ 0.56 & 0.44 \end{pmatrix}$$
>
> 計算過程：$(0.8 \times 0.8 + 0.2 \times 0.4) = 0.72$，$(0.8 \times 0.2 + 0.2 \times 0.6) = 0.28$ ...
>
> 所以今天晴天 → 後天晴天的機率 = 0.72，後天雨天 = 0.28。
>
> **求穩態分布（2-state 完整推導）**：
>
> 設 $\pi = (\pi_S, \pi_R)$，要滿足 $\pi \mathbf{P} = \pi$ 且 $\pi_S + \pi_R = 1$。
>
> 展開：$\pi_S = 0.8\pi_S + 0.4\pi_R$
>
> $0.2\pi_S = 0.4\pi_R \implies \pi_S = 2\pi_R$
>
> 加上 $\pi_S + \pi_R = 1$：$2\pi_R + \pi_R = 1 \implies \pi_R = \frac{1}{3}$，$\pi_S = \frac{2}{3}$
>
> **結論**：長期而言，約 66.7% 的天數是晴天，33.3% 是雨天。不管今天是什麼天氣，很多天以後天氣分布都會接近這個比例。
...
> **數值範例：3x3 轉移矩陣**
>
> 一個學生的每日學習狀態：用功(H)、普通(M)、偷懶(L)。
>
> $$\mathbf{P} = \begin{pmatrix} 0.5 & 0.4 & 0.1 \\ 0.3 & 0.4 & 0.3 \\ 0.1 & 0.3 & 0.6 \end{pmatrix}$$
>
> 讀法：如果今天「用功」，明天有 50% 機率還是用功、40% 普通、10% 偷懶。
>
> 檢查：每行加起來 = 1 ✓ ($0.5+0.4+0.1=1$, $0.3+0.4+0.3=1$, $0.1+0.3+0.6=1$)
>
> 如果今天偷懶，明天轉成用功的機率只有 10%，但轉成用功後隔天繼續用功的機率有 50%。這符合直覺：「好習慣」比較容易維持，但「起步」比較難。
...
> **數值範例：Gambler's Ruin 完整計算**
>
> **場景**：小明帶了 $i = 3$ 元去賭場，目標是贏到 $N = 8$ 元就走。每局贏 1 元的機率 $p = 0.45$，輸 1 元的機率 $q = 0.55$。
>
> **問題 1**：小明破產的機率是多少？
>
> $q/p = 0.55/0.45 = 11/9 \approx 1.2222$
>
> $$r_3 = \frac{(11/9)^3 - (11/9)^8}{1 - (11/9)^8}$$
>
> 計算各項：
> - $(11/9)^3 = 1.2222^3 \approx 1.8266$
> - $(11/9)^8 = 1.2222^8 \approx 5.1598$
>
> $$r_3 = \frac{1.8266 - 5.1598}{1 - 5.1598} = \frac{-3.3332}{-4.1598} \approx 0.8013$$
>
> **小明有約 80% 的機率會破產！**
>
> **問題 2**：如果是公平遊戲（$p = q = 0.5$），結果呢？
>
> $$r_3 = \frac{N - i}{N} = \frac{8 - 3}{8} = \frac{5}{8} = 0.625$$
>
> 即使公平遊戲，帶 3 元想贏到 8 元，破產機率仍有 62.5%！因為你帶的錢比目標少很多。
>
> **問題 3**：公平遊戲下，期望遊戲步數？
>
> $$D_3 = i(N-i) = 3 \times (8-3) = 3 \times 5 = 15 \text{ 步}$$
>
> **教訓**：賭場優勢（$p < 0.5$）+「小資金挑戰大目標」= 幾乎必然破產。這就是為什麼「久賭必輸」。
...
1. **Poisson Process 基礎題**：某醫院急診室平均每小時來 8 位病人（Poisson Process）。(a) 30 分鐘內恰好來 3 位的機率？(b) 兩位病人之間的平均等待時間？

2. **Markov Chain 題**：一個 2-state Markov Chain，轉移矩陣為 $\mathbf{P} = \begin{pmatrix} 0.7 & 0.3 \\ 0.5 & 0.5 \end{pmatrix}$。求穩態分布。

3. **Gambler's Ruin 題**：公平遊戲（$p = 0.5$），初始金額 4 元，目標 10 元。求破產機率和期望遊戲步數。

4. **Poisson 分拆題**：一家書店平均每小時來 10 位顧客。每位顧客獨立地有 30% 的機率會買書。求 1 小時內買書的顧客人數的分布。

<details><summary>參考答案</summary>

**1.** (a) 30 分鐘 = 0.5 小時，$N(0.5) \sim \text{Poisson}(8 \times 0.5) = \text{Poisson}(4)$。
$P(N=3) = \frac{4^3 e^{-4}}{3!} = \frac{64 \times 0.0183}{6} = \frac{1.1725}{6} \approx 0.1954$

(b) 間隔時間 $T \sim \text{Exp}(8)$，$E[T] = 1/8$ 小時 = **7.5 分鐘**

**2.** $\pi_1 = 0.7\pi_1 + 0.5\pi_2$，$\pi_1 + \pi_2 = 1$。
$0.3\pi_1 = 0.5\pi_2 \implies \pi_1 = \frac{5}{3}\pi_2$。
$\frac{5}{3}\pi_2 + \pi_2 = 1 \implies \frac{8}{3}\pi_2 = 1 \implies \pi_2 = \frac{3}{8}$，$\pi_1 = \frac{5}{8}$。
$\pi = (5/8, 3/8) = (0.625, 0.375)$

**3.** $p = 0.5$，$i = 4$，$N = 10$。
破產機率：$r_4 = (N-i)/N = 6/10 = 0.6 = 60\%$
期望步數：$D_4 = i(N-i) = 4 \times 6 = 24$ 步

**4.** 由 Poisson 分拆：顧客流（速率 10）中，每人以機率 0.3 獨立地「買書」。
買書的顧客數 $\sim \text{Poisson}(10 \times 0.3) = \text{Poisson}(3)$。
不買書的 $\sim \text{Poisson}(7)$，且兩者獨立。
