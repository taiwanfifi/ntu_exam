...
想像你打開 Google Maps，輸入起點和終點，它幫你規劃最快路線——這就是最短路徑演算法在背後運作。不只是導航，物流配送、網路封包路由、社群網路中的「六度分隔」計算，全部都依賴最短路徑演算法。

**具體場景：** 你住在 A 城，想開車到 E 城。中間有 B、C、D 三個城市，城市之間有不同的公路（有的遠、有的近）。你想找到從 A 到 E 的最短行車路線。這就是「單源最短路問題 (SSSP)」。如果你還想知道任意兩個城市之間的最短距離（例如做一張完整的距離表），那就是「全對最短路問題 (APSP)」。

### 本章關鍵術語表

| 術語 | 英文 | 白話解釋 | 例子 |
|------|------|----------|------|
| 鬆弛 | Relaxation | 如果發現經由另一個城市中轉比直接走更短，就更新路線 | 直飛台北→紐約要 16 小時，但台北→東京→紐約只要 14 小時，那就更新為 14 小時 |
| 單源最短路 | SSSP (Single-Source Shortest Paths) | 從一個固定起點到所有其他點的最短距離 | 從你家到所有餐廳的最短距離 |
| 全對最短路 | APSP (All-Pairs Shortest Paths) | 任意兩點之間的最短距離 | 所有城市之間的距離表 |
| 負權邊 | Negative weight edge | 權重為負數的邊（走過去反而「賺到」距離） | 某條路有補貼，走過去成本為負 |
| 負環 | Negative cycle | 繞一圈後總距離為負的環路 | 反覆走這個環可以讓距離無限小，導致最短路無意義 |
| 前驅節點 | Predecessor (pred) | 最短路上，到達某點之前經過的上一個點 | 最短路 A→C→B 中，B 的前驅是 C |
| 估計距離 | dist[v] | 目前已知的從起點到 v 的最短距離估計值 | 演算法過程中不斷更新，最終收斂為真正最短距離 |

### 前置知識

- **圖的基本概念**（algo_01~02）：頂點、邊、有向圖、無向圖、鄰接表
- **BFS / DFS**（algo_02）：圖的遍歷方式
- **Greedy 演算法**（algo_04）：Dijkstra 本質上是 Greedy
- **Dynamic Programming**（algo_03）：Floyd-Warshall 是 DP 的經典應用
- **拓撲排序**（algo_02）：DAG 最短路需要用到

### Relaxation（鬆弛）的日常比喻

> **生活比喻：** 假設你知道從台北直飛到紐約要花 30,000 元。有一天你發現「台北→東京只要 8,000 元，東京→紐約只要 15,000 元」，加起來 23,000 元比 30,000 元便宜！於是你**更新**（relax）你的估計：台北到紐約的最便宜方式是 23,000 元，經由東京中轉。
>
> 這就是 Relaxation 的全部概念：**如果經由中間城市 B 到目的地 C 比目前已知的路線更短，就更新路線。**
...
你要求什麼？
│
├── 只需要「一個起點」到其他所有點的最短距離？（SSSP）
│   │
│   ├── 圖是 DAG（有向無環圖）嗎？
│   │   └── Yes → 用「拓撲排序 + Relaxation」 ⏱ O(V+E) ✅ 最快！可處理負權
│   │
│   ├── 所有邊權都 ≥ 0 嗎？
│   │   └── Yes → 用 Dijkstra ⏱ O((V+E) log V)
│   │
│   └── 有負權邊嗎？
│       └── Yes → 用 Bellman-Ford ⏱ O(VE)（還能偵測負環）
│
└── 需要「所有點對」之間的最短距離？（APSP）
    │
    ├── 稠密圖（邊很多，E ≈ V²）？
    │   └── 用 Floyd-Warshall ⏱ O(V³)
    │
    └── 稀疏圖（邊很少）？
        └── 用 Johnson's ⏱ O(V² log V + VE)
```
...
> **🔰 白話比喻：** 你原本打算直接從台北搭飛機到紐約（花 30,000 元）。朋友告訴你：「我剛查到台北到東京 8,000 元、東京到紐約 15,000 元，加起來才 23,000 元！」你心想 23,000 < 30,000，趕緊把計畫改成經由東京轉機。這個「發現更便宜的中轉路線就更新」的動作，就是 Relaxation。
>
> 用偽代碼表達：
> ```
> 如果 dist[紐約] > dist[東京] + 機票(東京→紐約):
>     dist[紐約] = dist[東京] + 機票(東京→紐約)
>     前驅[紐約] = 東京
> ```
...
> **🔰 白話解釋為什麼 Dijkstra 不能處理負權：**
>
> Dijkstra 的邏輯是「我從優先佇列裡拿出距離最小的點 C，它的距離一定是最終答案了，因為其他還沒處理的點距離都比 C 大，不可能繞過來讓 C 更短。」
>
> 但如果有**負權邊**，這個邏輯就崩了！因為一個距離更大的點 D（還沒被拿出來），可能有一條負權邊 D→C，使得「繞過 D 到 C」反而更短。C 已經被「鎖定」了，來不及更新。
>
> **一句話總結：** Dijkstra 像是「先到先贏」——先被拿出來的點就定案了。負權邊會讓「後到的選手」反超「已經定案的選手」，破壞這個規則。
>
> **⚠️ 常見錯誤：** 有些同學以為「只要負權邊很少，Dijkstra 就沒問題」。**錯！** 只要有一條負權邊，Dijkstra 就可能出錯。即使圖中 99% 的邊都是正的，那 1% 的負權邊就可能導致錯誤結果。有負權邊時，請改用 Bellman-Ford。
...
1. **Relaxation 是什麼意思？** 請用一句白話解釋，並寫出 Relaxation 的偽代碼。

2. **Dijkstra 為什麼不能處理負權邊？** 請用你自己的話解釋（不要只說「因為 greedy」），並舉一個具體的 3 個節點反例。

3. **Bellman-Ford 為什麼要做 V-1 輪？** 如果最短路只有 2 條邊，做 2 輪就夠了嗎？

4. **Floyd-Warshall 的迴圈順序為什麼 k 要在最外層？** 如果寫成 `for i, for j, for k` 會怎樣？

5. **Johnson's Algorithm 的核心想法是什麼？** 為什麼不直接跑 V 次 Dijkstra？

### 計算題

6. 對以下有向圖，從 S 出發，手動執行 Dijkstra 演算法。寫出每步的 dist[]、pred[]、visited 集合。
   ```
   S→A:4, S→B:2, A→C:3, B→A:1, B→C:6, C→D:1, A→D:5
   ```

7. 對以下有向圖（含負權邊），從 S 出發，手動執行 Bellman-Ford。假設邊的遍歷順序為 (S,A), (S,B), (A,B), (A,C), (B,C)。
   ```
   S→A:3, S→B:5, A→B:-2, A→C:4, B→C:1
   ```

8. 對以下 3 個頂點的有向圖，手動執行 Floyd-Warshall，寫出 $D^{(0)}, D^{(1)}, D^{(2)}, D^{(3)}$。
   ```
   1→2:4, 1→3:11, 2→3:2, 3→1:6
   ```

### 判斷題（True/False，並說明理由）

9. 「如果圖中所有邊權都是正整數，Bellman-Ford 和 Dijkstra 會得到相同的結果。」

10. 「Floyd-Warshall 可以偵測負環。」

11. 「在 DAG 上，即使有負權邊，也可以用拓撲排序 + Relaxation 在 O(V+E) 時間求最短路。」

12. 「Johnson's Algorithm 對稠密圖（E ≈ V²）比 Floyd-Warshall 快。」

### 參考答案提示

- 第 6 題：最終 dist = [S:0, A:3, B:2, C:6, D:7]
- 第 7 題：第 1 輪後 dist = [S:0, A:3, B:1, C:2]（注意 A→B 的負權邊讓 B 的距離從 5 更新為 1）
- 第 9 題：True。兩者在非負權圖上都能正確求解 SSSP，結果相同（但效率不同）。
- 第 12 題：False。稠密圖時 Johnson's 是 $O(V^2 \log V + V \cdot V^2) = O(V^3)$，和 Floyd-Warshall 的 $O(V^3)$ 差不多，甚至常數更大。Johnson's 在**稀疏圖**才有優勢。
